...
promzard(pkgIn, pzCtx, function(err, ctx) {
  ...
  writeTemplate('travis.yml', ctx, '.travis.yml');
  writeTemplate('Makefile', ctx);

  writeTemplateDir('src', ctx);
  writeTemplateDir('test', ctx);
...
function writeTemplateDir(dir, ctx) {
  var files = fs.readdirSync(path.join(templateDir, dir));
  files.forEach(function(file) {
    file = path.join(dir, file);
    var stat = fs.statSync(path.join(templateDir, file));
    writeTemplate(file, ctx, file, stat.mode & 0x1FF);
  });
}

function writeTemplate(src, ctx, dest, mode) {
  dest = dest || src;
  writeFile(_.template(readFile(path.join(templateDir, src)), ctx), path.join(cwd, dest), mode);
}
