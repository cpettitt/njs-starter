...
promzard(pkgIn, pzCtx, function(err, ctx) {
  ...
  var license = readFile(getLicenseFile(ctx.license));
  ctx.licenseFile = license;

  writePackageJson(ctx);
  writeFile(_.template(license, ctx), cwd, 'LICENSE');
  writeFile(_.template(readFile(templateDir, 'README.md'), ctx), cwd, 'README.md');
  writeFile(_.template(readFile(templateDir, 'CHANGELOG.md'), ctx), cwd, 'CHANGELOG.md');
